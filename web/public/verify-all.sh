#!/bin/bash
# Habitat Portfolio Audit - Portable Verification
# Fetches proofs from https://habitat-za.vercel.app/ using manifest

set -e

echo "=== Habitat Portfolio Audit Verification ==="
echo "Date: $(date)"
echo ""

# Create temporary verifier
VERIFIER=$(mktemp /tmp/habitat_verify.XXXXXX.py)
cat > "$VERIFIER" << 'VERIFIER_EOF'
import base64
import hashlib
import json
import math
...
# Fetch manifest
MANIFEST_URL="https://habitat-za.vercel.app/proofs-manifest.json"
echo "Fetching manifest from $MANIFEST_URL..."

MANIFEST=$(curl -sL "$MANIFEST_URL")
if [ -z "$MANIFEST" ] || ! echo "$MANIFEST" | python3 -m json.tool > /dev/null 2>&1; then
    echo "ERROR: Could not fetch valid manifest from $MANIFEST_URL"
    rm -f "$VERIFIER"
    exit 1
fi

# Parse projects
PROJECTS=$(echo "$MANIFEST" | python3 -c "
import sys, json
...
for project in $PROJECTS; do
    ...
    RUNS=$(echo "$MANIFEST" | python3 -c "
import sys, json
data = json.load(sys.stdin)
project = sys.argv[1]
for run in data['projects'][project]:
    print(run['run_id'])
" "$project")
    
    for run in $RUNS; do
        PROOF_URL=$(echo "$MANIFEST" | python3 -c "
import sys, json
data = json.load(sys.stdin)
project = sys.argv[1]
run_id = sys.argv[2]
...
" "$project" "$run")
        ...
        # Download proof to temp file, then verify
        PROOF_TMP=$(mktemp /tmp/habitat_proof.XXXXXX.json)
        if curl -sL "$PROOF_URL" -o "$PROOF_TMP" && python3 "$VERIFIER" "$PROOF_TMP" > /dev/null 2>&1; then
            echo "PASS"
            PASSED=$((PASSED + 1))
        else
            echo "FAIL"
...
echo "Failed: $FAILED"
if [ $TOTAL -gt 0 ]; then
    PCT=$(python3 -c "import sys; print(float(sys.argv[1]) * 100 / float(sys.argv[2]))" "$PASSED" "$TOTAL")
...
if [ $FAILED -eq 0 ] && [ $TOTAL -gt 0 ]; then
    ...
    echo ""
    echo "SOME PROOFS FAILED OR NONE FOUND"
    exit 1
fi
