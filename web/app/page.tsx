  ...
  return (
    <main className="shell">
      ...
      <section className="hero">
        <p className="eyebrow">{status.name}</p>
        <h1>Agent actions you can inspect, correlate and prove.</h1>
        <p className="lede">Habitat keeps the durable ledger and policy engine in the persistent service. Vercel provides this fast, public-facing inspection surface.</p>
        <div className="flow"><span>Agent</span><i>→</i><span>Event</span><i>→</i><span>Ledger</span><i>→</i><span>Verify</span><i>→</i><span>Proof</span></div>
      </section>
      <section className="metrics" aria-label="Habitat status">
        <Metric label="Jobs" value={status.jobs} />
        <Metric label="Agents" value={status.agents} />
        <Metric label="Active signals" value={status.active_signals} />
      ...
      <footer>Vercel is the presentation layer. Habitat remains the source of truth.</footer>
    </main>
  );
}
...
function ClaimCard({ claim }: { claim: Claim }) {
  ...
  return <article className="claim-card">
    <div className="claim-top"><span className={`status ${verified ? "ok" : "attention"}`}>{claim.status || "unknown"}</span><span className="mono">{claim.id}</span></div>
    <h3>{claim.statement || "Claim without a statement"}</h3>
    <div className="meta"><span>Agent <b>{claim.agent_id || "—"}</b></span><span>Run <b>{claim.run_id || "—"}</b></span><span>Expected <b>{claim.expected_status || "—"}</b></span></div>
    <div className="actions"><Link className="button primary" href={`/claims/${encodeURIComponent(claim.id)}`}>Inspect claim</Link><a className="button" href={`/api/claims/${encodeURIComponent(claim.id)}/verify`}>Verify now</a></div>
  </article>;
}
