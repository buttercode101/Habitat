...
<meta name="theme-color" content="#090a0c"><meta name="description" content="Habitat — local-first supervision and portable evidence for autonomous agents.">
...
* {{ box-sizing:border-box }} html {{ scroll-behavior:smooth }} body {{ margin:0;background:var(--bg);color:var(--text);font-family:Inter,ui-sans-serif,system-ui,-apple-system,BlinkMacSystemFont,"Segoe UI",sans-serif;line-height:1.55 }}
a {{ color:inherit;text-decoration:none }} .container {{ width:min(1120px,calc(100% - 40px));margin:0 auto }} .mono {{ font-family:ui-monospace,SFMono-Regular,Menlo,monospace }}
.nav {{ height:72px;display:flex;align-items:center;justify-content:space-between;border-bottom:1px solid var(--line) }} .brand {{ display:flex;align-items:center;gap:10px;font-weight:760;letter-spacing:-.02em }}
...
footer {{ border-top:1px solid var(--line);padding:28px 0 44px;color:var(--muted);font-size:13px;display:flex;justify-content:space-between;gap:20px }} .dashboard {{ padding:36px 0 64px }} .dashhead {{ display:flex;justify-content:space-between;gap:24px;align-items:flex-end;margin-bottom:24px }}
.dashhead h1 {{ font-size:30px;letter-spacing:-.04em;margin:0 0 4px }} .dashsub {{ color:var(--muted);font-size:13px }} .kpis {{ display:grid;grid-template-columns:repeat(4,1fr);gap:10px;margin-bottom:12px }}
.kpi,.panel {{ border:1px solid var(--line);background:var(--surface);border-radius:12px }} .kpi {{ padding:18px }} .kpi .value {{ font-size:30px;font-weight:720;letter-spacing:-.04em }} .kpi .value small {{ color:var(--muted);font-size:14px;font-weight:500 }} .kpi .label {{ color:var(--muted);font-size:12px;margin-top:3px }}
...
@media(max-width:460px) {{ h1 {{ font-size:46px }} .cards,.kpis {{ grid-template-columns:1fr }} footer {{ flex-direction:column }} }} {extra_css}
