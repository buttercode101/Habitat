...
<div class="panel"><h2>Proof digest</h2><code>{_value(digest)}</code><div class="actions"><a class="button" href="/v1/claims/{escape(str(payload.get('claim_id','')) )}/verify" target="_blank" rel="noreferrer">Verify claim</a><a class="button alt" href="/v1/claims/{escape(str(payload.get('claim_id','')) )}/proof" target="_blank" rel="noreferrer">View proof JSON</a></div></div></section>
<aside><div class="panel"><h2>Assurance</h2>{checks}</div><div class="panel"><h2>Cryptographic status</h2><div class="meta"><div>Signature<b>{signature_text}</b></div><div>Publisher trust<b>{_value(assurance.get('publisher_trust'))}</b></div></div></div><div class="panel"><p class="note">A valid proof establishes integrity and consistency within Habitat's evidence boundary. It does not by itself establish that the publisher is trusted or that the underlying real-world event is true.</p></div></aside></div></main></body></html>"""
    return body
